...
std::string getFilename() {
    ...
    oss << "dxgi_nvenc_" << std::put_time(&tm, "%Y%m%d_%H%M%S") << ".mp4";
    return oss.str();
}
...
int main() {
    ...
    if (!codec) { std::cerr << "NVENC H264 codec not found\n"; return -1; }
    ...
    codecCtx->thread_count = 1; // NVENC uses GPU threads
    ...
    std::cout << "NVENC recording finished: " << filename << "\n";
    return 0;
}
