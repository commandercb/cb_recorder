...
int main() {
    ...
    std::thread encoderThread([&]() {
        ...
        while (!stopRecording.load() || !frameQueue.empty()) {
            ...
            while (true) {
                ...
                av_packet_unref(pkt);
                if (avio_tell(outCtx->pb) >= 3900000000) stopRecording = true;
            }
        }
    ...
    std::cout << "Recording (DXGI 60 FPS)... Ctrl+C to stop\n";
    ...
    // ---------------- FINALIZE WITH FFMPEG ----------------
    {
        ...
        finalName.replace(finalName.find(".avi"), 4, "_final.mp4");
        ...
        std::cout << "Finalizing file with FFmpeg...\n";
        system(cmd.str().c_str());
        std::cout << "Final file written: " << finalName << "\n";
    }
